...
class Model{
	...
	constructor(gl, programInfoSlice, programInfo, positions, indices, textureCoordinates, vertexNormals, textureUrl, ifNotRay){
		...
		if(ifNotRay){
			...
			//mat4.scale(this.modelViewMatrix, this.modelViewMatrix, [0.02, 0.02, 0.02]);
		}
		...
		//mat4.perspective(this.projectionMatrixSlice, (45 * Math.PI / 180), 1, 0.1, 100);
		mat4.ortho(this.projectionMatrixSlice, -1, 1, -1, 1, 0, 100);
	...
	findRecenterVector(positions){
		...
		//console.log("length:"+positions.length+" reduced:"+positions.length/3);
		var recenterVector = [sumX/(positions.length/3), sumY/(positions.length/3)];
		//console.log(recenterVector);
	...
	draw(){
		...
		//textureCoord
		this.gl.bindBuffer(this.gl.ARRAY_BUFFER, this.textureCoordBuffer);
		//                          shaderLocation           choordsHave2Values  type     normalize  stride offset
		...
		//tell webgl witch texture to use texture unit 0 ?????????
		//this.gl.activeTexture(this.gl.TEXTURE0);
		//this.gl.bindTexture(this.gl.TEXTURE_2D, this.texture);
		this.gl.uniform1i(this.programInfo.unifromLocations.uSampler, 0);
	...
	translateSlice(amount){
		...
		//mat4.perspective(this.projectionMatrixSlice, (45 * Math.PI / 180), 1, 0.1+this.sliceAmount, 100);
